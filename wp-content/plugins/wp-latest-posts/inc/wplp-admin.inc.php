...
class WPLPAdmin
{
    ...
    public function __construct($opts)
    {
        ...
        add_action('wp_ajax_wplp_get_list_block', array($this, 'wplpGetListBlock'));
        if (is_admin()) {
            /**
     ...
     * @return void
     */
    public function wplpGetListBlock()
    {
        if (!empty($_REQUEST['wplp_get_list_block'])) {
            $blocks                   = get_posts(
                array(
            ...
            $html = '<option value="0">'.esc_html__('Choose a block', 'wp-latest-posts').'</option>';
            foreach ($blocks as $block) {
                $html .= '<option value="'.$block->ID.'">'.$block->post_title.'</option>';
            }
            ...
            wp_send_json(
                array(
                    'success'   => true,
                    'list'      => $html,
                    'adminUrl'  => esc_url(admin_url('admin.php?page=wplp-widget'))
                )
            );
        }
        ...
        if (!empty($_REQUEST['id'])) {
            ...
            if (isset($widget) && !empty($widget)) {
                ...
                if (!empty($_REQUEST['divi'])) {
                    ...
                    $widget->settings['addon_enable'] = $addon_enable;
                    $widget->settings['pausehover'] = $pausehover;
                    $widget->settings['pauseaction'] = $pauseaction;
                    $data_array = array(
                        'id'               => $id,
                        'nbcol'            => $nbcol,
                        'nbrow'            => $nbrow,
                    ...
                    $html .= '<script>var WPLP_'.$id.'='.json_encode($data_array).';</script>';
                    if ($widget->settings['theme'] !== 'default' && defined('WPLP_ADDON_VERSION')) {
                        if ($widget->settings['theme'] === 'masonry' || $widget->settings['theme'] === 'masonry-category' || $widget->settings['theme'] === 'material-vertical' || $widget->settings['theme'] === 'material-horizontal') {
                            $html .= '<script src="'. WPLPADDON_PLUGIN_DIR .'js/wplp_addon_front.js?ver='.WPLP_VERSION .'"></script>';// phpcs:ignore
                        }

                        $ajax_non = wp_create_nonce('wplp-addon-front-nonce');
                        $html .= '<script id="wplp_addon_front-js-extra">var wpsolAddonFrontJS = {"ajaxnonce":"'.$ajax_non.'"};</script>';
                    }
                }
    ...
    public function saveCustomPost($post_id, $type)
    {
        ...
        if ($my_settings['load_more'] === '1' && $my_settings['theme'] !== 'default' && $my_settings['theme'] !== 'smooth-effect') {
            if ($my_settings['max_elts'] < $my_settings['per_page']) {
                wp_safe_redirect(admin_url('admin.php?page=wplp-widget&view=block&id=' . $post_id . '&save_block=unsuccessful'));
                return true;
            }
        }
        ...
        if (is_array($my_settings['source_category']) && in_array('_all', $my_settings['source_category'])) {
            $my_settings['source_category'] = array('_all');
        }
        if (is_array($my_settings['source_category_list']) && in_array('_all', $my_settings['source_category_list'])) {
            $my_settings['source_category_list'] = array('_all');
        }
        if (is_array($my_settings['source_pages']) && in_array('_all', $my_settings['source_pages'])) {
            $my_settings['source_pages'] = array('_all');
        }
        if (is_array($my_settings['source_tags']) && in_array('_all', $my_settings['source_tags'])) {
            $my_settings['source_tags'] = array('_all');
        }
        ...
        if ($my_settings['dfThumbnail'] === '' && $my_settings['dfTitle'] === '' && $my_settings['dfAuthor'] === '' && $my_settings['dfText'] === '' && $my_settings['dfDate'] === '' && $my_settings['dfCategory'] === '' && $my_settings['dfReadMore'] === '') {
            wp_safe_redirect(admin_url('admin.php?page=wplp-widget&view=block&id=' . $post_id . '&save_block=success&content=empty'));
            return true;
        }
    ...
    public function addStylesheet()
    {
        ...
        */
        ...
        if (file_exists($myStylePath)) {
            wp_register_style('myStyleSheets', $myStyleUrl, array(), WPLP_VERSION);
            wp_enqueue_style('myStyleSheets');
        }

        // load addon theme style for Elementor page earlier
        if (class_exists('\\Elementor\\Plugin')) {
            ...
            if (is_singular()) {
                ...
                if (isset($elementor_data[0])) {
                    ...
                    // var_dump($elementor_data[0]);
                    $regExp = '/"widgetType":"([^"]*)/i';
                    $outputArray = array();

                    foreach ($outputArray as $el) {
                        if ($el[1] === 'wplp') {
                            $found = true;
                        }
                    }
                    ...
                    if ($found) {
                        $regExp2 = '/"news_widget_id":"([^"]*)/i';
                        $outputArray2 = array();

                        if (!empty($outputArray2)) {
                            $addon_theme_dir  = apply_filters('wplp_plugindir', '');
                        }
                        foreach ($outputArray2 as $el) {
                            $widgetSettings = get_post_meta($el[1], '_wplp_settings', true);
                            $themeName = basename($widgetSettings['theme']);
                            if ($themeName !== 'default') {
                                $theme_path = $addon_theme_dir . '/' . $themeName;
                                if (file_exists($theme_path)) {
                                    $theme_hanlde = 'themes-wplp-' . $themeName;
                                    $theme_style_url = plugins_url('wp-latest-posts-addon/themes/') . $themeName .'/style.css';
                                    wp_register_style($theme_hanlde, $theme_style_url, array(), WPLP_VERSION);
                                    wp_enqueue_style($theme_hanlde);
                                }
                            }
                        }
                    }
        ...
        } // end fix load css file for Elementor
    }
    ...
    public function addEditorAssets()
    {
        ...
        wp_enqueue_style('wplpStyleDefault', plugins_url('themes/default/style.css', dirname(__FILE__)), array(), WPLP_VERSION);
        if (defined('WPLP_ADDON_VERSION')) {
            wp_enqueue_style('wplpStyleMasonry', WPLPADDON_PLUGIN_DIR . 'themes/masonry/style.css', array(), WPLP_VERSION);
            wp_enqueue_style('wplpStyleMaterialVertical', WPLPADDON_PLUGIN_DIR . 'themes/material-vertical/style.css', array(), WPLP_VERSION);
            wp_enqueue_style('wplpStyleMaterialHorizontal', WPLPADDON_PLUGIN_DIR . 'themes/material-horizontal/style.css', array(), WPLP_VERSION);
            wp_enqueue_style('wplpStyleMasonryCategory', WPLPADDON_PLUGIN_DIR . 'themes/masonry-category/style.css', array(), WPLP_VERSION);
            wp_enqueue_style('wplpStyleSmooth', WPLPADDON_PLUGIN_DIR . 'themes/smooth-effect/style.css', array(), WPLP_VERSION);
            wp_enqueue_style('wplpStyleTimeline', WPLPADDON_PLUGIN_DIR . 'themes/timeline/style.css', array(), WPLP_VERSION);
            wp_enqueue_style('wplpStylePortfolio', WPLPADDON_PLUGIN_DIR . 'themes/portfolio/style.css', array(), WPLP_VERSION);

            wp_enqueue_script(
                'wplp-flexslider-js',
                plugins_url('js/flexslider.min.js', dirname(__FILE__)),
        ...
        wp_enqueue_script(
            'wplp_blocks',
            plugins_url('js/blocks/insert-news-block/block.js', dirname(__FILE__), array(), WPLP_VERSION),
            array('wp-blocks', 'wp-i18n', 'wp-element', 'wp-data', 'wp-editor'),
            WPLP_VERSION
        );
    ...
    public function loadAdminScripts($hook)
    {
        ...
        if ($current_page->base === 'latest-posts_page_wplp-configuration' ||
            ...
            wp_enqueue_style(
                'wplp-material-icon',
                plugins_url('css/fonts/material/material-icons.css', dirname(__FILE__), array(), WPLP_VERSION)
            );
            wp_register_style('wplp_framework_css', plugins_url('wordpress-css-framework/css/style.css', dirname(__FILE__)), array(), WPLP_VERSION);
            wp_enqueue_style('wplp_framework_css');

            wp_register_style('wplp_framework_waves', plugins_url('wordpress-css-framework/css/waves.css', dirname(__FILE__)), array(), WPLP_VERSION);
        ...
        if ($current_page->base === 'latest-posts_page_wplp-configuration') {
            ...
            wp_register_style('wplpAbout', plugins_url('css/wplp_about.css', dirname(__FILE__)), array(), WPLP_VERSION);
        ...
        wp_enqueue_style('wplp-tooltipster', plugins_url('css/tooltipster.bundle.min.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('wplp-tooltipster-theme', plugins_url('css/tooltipster-sideTip-borderless.min.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_script('wplp-toWPLP_VERSIONWPLP_VERSIONWPLP_VERSIONoltipster', plugins_url('js/tooltipster.bundle.min.js', dirname(__FILE__)), array(), WPLP_VERSION, true);
    }
    ...
    public function addAdminStylesheets()
    {
        ...
        wp_register_style('uiStyleSheet', plugins_url('css/jquery-ui-custom.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('uiStyleSheet');

        wp_register_style('wplp_dropify', plugins_url('js/dropify/css/dropify.min.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('wplp_dropify');

        wp_register_style('wplpPicker', plugins_url('css/picker.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('wplpPicker');

        wp_register_style('wplpAdmin', plugins_url('css/wplp_admin_widget.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('wplpAdmin');

        wp_register_style('wplpAdminSettings', plugins_url('css/wplp_admin_block.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('wplpAdminSettings');

        wp_register_style('wplpAdminQuirk', plugins_url('css/quirk.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('wplpAdminQuirk');

        wp_register_style('unifStyleSheet', plugins_url('css/uniform/css/uniform.default.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('unifStyleSheet');

        /**
         * Add codemirror css
         */
        wp_register_style('wplp_codemirror', plugins_url('codemirror/lib/codemirror.css', dirname(__FILE__)), array(), WPLP_VERSION);
        wp_enqueue_style('wplp_codemirror');

        wp_register_style('wplp_codemirrorTheme', plugins_url('codemirror/theme/3024-day.css', dirname(__FILE__)), array(), WPLP_VERSION);
    ...
    public function getCountPosts()
    {
        ...
        if (isset($_POST['wplp_id']) && is_numeric($_POST['wplp_id'])) {
        ...
        $front = new WPLPFront($widget);
        $count_posts = count($front->posts);
        switch ($widget->settings['source_type']) {
            case 'src_category':
                if (empty($widget->settings['source_category'])) {
                    $count_posts = 0;
                }
                break;
            case 'src_category_list':
                if (empty($widget->settings['source_category_list'])) {
                    $count_posts = 0;
                }
                break;
            case 'src_page':
                if (empty($widget->settings['source_pages'])) {
                    $count_posts = 0;
                }
                break;
            case 'src_tags':
                if (empty($widget->settings['source_tags'])) {
                    $count_posts = 0;
                }
                break;
            default:
        ...
        if ($count_posts > 1) {
            ...
            $text = esc_html__(' Post', 'wp-latest-posts');
        }
        ...
        switch ($settings['wplp_source_type']) {
            case 'src_category_list':
                if ($count_posts > 1) {
                    ...
                    $text = esc_html__(' Category', 'wp-latest-posts');
                }
                break;
            case 'src_page':
                if ($count_posts > 1) {
                    ...
                    $text = esc_html__(' Page', 'wp-latest-posts');
                }
                break;
        }
        wp_send_json(array('status' => true, 'count' => $count_posts, 'text' => $text));
    }
    ...
    public function applyShortcode($args = array())
    {
        ...
        } elseif (function_exists('FusionBuilder') && (is_admin() === true || fusion_is_builder_frame() === true)) {
            $elementor_preview_active = true;
        }
        ...
        if (isset($widget) && !empty($widget)) {
            ...
            if ($elementor_preview_active) {
                $html .= '<style>' . $front->loadThemeStyle(true) . '</style>';
                $script = '';
                $stylesheet = '';
                $type = 'stylesheet';
                ...
                $script .= '<script>var WPLP_'.$id.'='.json_encode($data_array).';</script>';
                ...
                if ($widget->settings['theme'] === 'default') {
                    ...
                    if (defined('WPLP_ADDON_VERSION')) {
                        ...
                        if ($widget->settings['theme'] === 'masonry' || $widget->settings['theme'] === 'masonry-category' || $widget->settings['theme'] === 'material-vertical' || $widget->settings['theme'] === 'material-horizontal') {
                            $script .= '<script src="'. WPLPADDON_PLUGIN_DIR .'js/wplp_addon_front.js?ver='.WPLP_VERSION .'"></script>';// phpcs:ignore
                            $script .= '<script src="'. WPLP_PLUGIN_DIR .'js/imagesloaded.pkgd.min.js?ver='.WPLP_VERSION .'"></script>';// phpcs:ignore
                        }
                        ...
                        if ($widget->settings['theme'] !== 'material-horizontal') {
                            $script .= '<script id="wplp-'. $widget->settings['theme'] .'-js" src="'. $scr_url .'"></script>';
                        }

                        $ajax_non = wp_create_nonce('wplp-addon-front-nonce');
                        $script .= '<script id="wplp_addon_front-js-extra">var wpsolAddonFrontJS = {"ajaxnonce":"'.$ajax_non.'"};</script>';
                        
                        $styles = $this->loadInlineStyle($widget->settings, $widget->ID);
                        $css = $styles['css'];
                        $str = $styles['str'];
                        $stylesheet = '<style>' . $css . $str . '</style>';
                    }
                }
                
                $stylesheet .= '<link rel="'. esc_attr($type) .'" href="'.esc_url($style).'" />';
                
                $html .= $stylesheet;
                $html .= $front->display(false);
                $html .= $script;
            } else {
                $front->loadThemeStyle(false);
                $html .= $front->display(false);
                // phpcs:ignore
                if (isset($_GET['et_pb_preview']) && ($widget->settings['theme'] === 'timeline' || $widget->settings['theme'] === 'portfolio')) {
                    $front->loadThemeScript(true);
                } else {
